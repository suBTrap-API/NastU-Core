...
public class Utils extends JsonUtils {
    ...
    public static List att_parse(JSONArray attachments){
        ...
        if(attachments.length() > 0){
            for(Object i: attachments){
                JSONObject att = JsonUtils.json(i.toString());
                String att_t = att.get("type").toString();
                if (Arrays.asList("link", "article").equals(att_t)) continue;
                atts.add(att_t+get(i.toString(), att_t).get("owner_id")+"_"+get(i.toString(), att_t).get("id"));
                if(get(i.toString(), att_t).has("access_key")){
                    atts.set(atts.size()-1, atts.get(atts.size() - 1)+ "_"+get(i.toString(), att_t).get("access_key"));
                }
            }
        }
        return atts;
