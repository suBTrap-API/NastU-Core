...
public class LongPool {
    ...
    static Logger logger = Logger.getLogger("LongPool");

    @SneakyThrows
    public LongPool(VkApi vkapi){
        vk = vkapi;
        String data = VkApi.call("messages.getLongPollServer");
        JSONObject json = Utils.json(data);
        if(Utils.json(data).has("error")){
            Utils.get(data, "error");
            if(Utils.getInt("error_code") == 5) throw new Exception("token-fail");
        }
    ...
    public List<Object> check() throws Exception {
        ...
        if(data.has("failed")){
            if((int) data.get("failed") == 1){
                ...
                key = Utils.json(VkApi.call("messages.getLongPollServer")).get("key").toString();
            } else {
