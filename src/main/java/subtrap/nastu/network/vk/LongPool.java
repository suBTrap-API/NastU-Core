...
public class LongPool {
    ...
    static Logger logger = new Logger("LongPoll");

    @SneakyThrows
    public LongPool(VkApi vkapi){
        vk = vkapi;
        String data = vk.call("messages.getLongPollServer");
    ...
    public List<Object> check() throws Exception {
        ...
        if(data.has("failed")){
            if((int) data.get("failed") == 1){
                ...
                key = new JSONObject(vk.call("messages.getLongPollServer")).get("key").toString();
            } else {
